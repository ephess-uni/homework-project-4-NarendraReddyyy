...
def reformat_dates(old_dates):
    ...
    return [datetime.strptime(i,"%Y-%m-%d").strftime("%d %b %Y") for i in old_dates]
...
def fees_report(infile, outfile):
    ...
    with open(outfile,'w',newline='') as file:
        ...
        late_fees_list=[{'patron_id':key,'late_fees':str(value)+'0' if len(str(value).split('.')[-1]) !=2 else str(value)} for key,value in late_fees_dict.items()]
